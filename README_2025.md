...
- Renamed SQL Arrays from SQL_to ARRAY_SQL_

2025-09-08-18-20-30

- Updated sql connections to use utf8m4 instead of utf8.
- Added new element DB type: eleValuesBadgesDB.
...
- Found Bug where Pickers would not set in a Portal.
...
- Added \xan\get->TAGS_ELE_INPUT_READONLY that sets inputs to readonly and changes the border and background to be different than an editable input.
- Updated Sales and SalesItems.
...
- Removed all router redundant calls to \xan\session_init in favor of one call.
...
- Added Constant for APP_RIGHT_CLICK_ALLOW.
...
- Added function redirectNow to \xan\response that uses an http 302 location header.
